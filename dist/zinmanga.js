(() => {
  ...
  // src/zinmanga.ts
  var exports_zinmanga = {};
  __export(exports_zinmanga, {
    search: () => search,
    getInfo: () => getInfo
  });
...
`) };
  ...
  // src/zinmanga.ts
  var baseUrl = "https://zinmanga.net";
  async function search(query, page = 1) {
    const { data: data2 } = await axios_default.get(`${baseUrl}/page/${page}?post_type=wp-manga&s=${encodeURIComponent(query)}`);
    const $2 = load(data2);
    const results = $2(".row.c-tabs-item__content").map((_, el) => {
      const link = $2(el).find(".post-title > h3 > a");
      const title = link.text().trim();
      const href = link.attr("href") || "";
      const id = href.split("/manga/")[1]?.replace("/", "");
      const image = $2(el).find("img").attr("data-src") || "";
      return {
        title,
        id,
        image
  ...
  async function getInfo(id) {
    const data2 = await axios_default.get(`${baseUrl}/manga/${id}`);
    const $2 = load(data2.data);
    const title = $2(".post-title > h1").text().trim();
    const image = $2(".summary_image > a > img").attr("src") || "";
    const chapters = $2(".wp-manga-chapter").text().trim();
    return {
      title,
      image,
      chapters
